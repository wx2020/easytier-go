...
| NET-07 | blocked | Implement QUIC plaintext compatibility. | **Descoped 2026-09-13** (see docs/GO_REWRITE_SE.md §11.4): the reference rides a patched quinn-plaintext stack with no maintained Go equivalent, so the `quic://` tunnel scheme is removed from the Go public transport surface (`ErrQuicTunnelDisabled`); config vocabulary stays compatible and the gateway-side QUIC proxy is unaffected. Restoring QUIC requires a real quinn-plaintext-interoperable implementation. |
