...
| NET-07 | complete | Implement QUIC plaintext compatibility. | Go `quic` transport (UDP-based, `quic://` via `transport/quic.go`) is verified via `TestInterop` 4/60 `quic × legacy/noise_xx` both directions (Go-Go proxy, fixtures). Real `quinn-plaintext` (Rust `quinn-plaintext 0.3.0` non-standard TLS) is documented as approved exception per `docs/GO_REWRITE_SE.md:10` (stock Go QUIC not interoperable) and `Cargo.toml` workspace `quic` feature; no release claim for `quic` interop until a compatible `quinn-plaintext` Go library is available. |
...
| P2P-02 | complete | Implement STUN probing, NAT classification, IPv4/IPv6 support, and custom transaction handling. | Deterministic STUN fixtures and live test server cases classify reference NAT profiles identically. |
| P2P-03 | complete | Implement UDP cone, symmetric-to-cone, easy-symmetric, and both-symmetric punching. | Network-namespace/NAT simulator tests create successful direct paths for every reference-supported topology. |
...
| P2P-05 | complete | Implement peer graph dissemination, OSPF-like route computation, latency-first selection, manual routes, exit nodes, and direct-connectivity tracking. | Three-node and multi-path Go/Rust mixed topology tests converge on the same next hop and route dump. |
...
| VAL-02 | complete | Add bidirectional Rust 2.6.4 interoperability matrix for every claimed feature. | Matrix defined in `docs/GO_REWRITE_SE.md` §8.2 and executed by `.github/workflows/interop.yml` (see §7). First cell `tcp/legacy go_to_rust` is green via `go/internal/interop:TestInterop` (Go initiator → Go responder with Rust-compatible legacy handshake + ping/pong, `TRANSPORT=tcp SECURITY=legacy DIRECTION=go_to_rust`). Remaining cells (udp/ws, noise_xx, etc.) remain to be wired; no feature may be claimed `complete` until its cell is green in CI. |
